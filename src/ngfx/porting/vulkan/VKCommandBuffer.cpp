...
void VKCommandBuffer::create(VkDevice device, VkCommandPool cmdPool, VkCommandBufferLevel level) {
    this->device = device; this->cmdPool = cmdPool;
    VkResult vkResult;
    allocateInfo = {
...
VKCommandBuffer::~VKCommandBuffer() {
    if (v) VK_TRACE(vkFreeCommandBuffers(device, cmdPool, 1, &v));
}

...
void VKFramebuffer::create(
    ...
    const std::vector<VKAttachmentDescriptor> &attachments, uint32_t w,
  ...
  createInfo = {VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO,
                NULL,
                0,
                renderPass,
                uint32_t(vkAttachments.size()),
                vkAttachments.data(),
                w,
                h,
                layers};
...
VKFramebuffer::~VKFramebuffer() {
  if (v)
    VK_TRACE(vkDestroyFramebuffer(device, v, nullptr));
}
...
Framebuffer *Framebuffer::create(Device *device, RenderPass *renderPass,
  ...
  std::vector<VKFramebuffer::VKAttachmentDescriptor> vkAttachments(
      attachments.size());
  for (int j = 0; j < vkAttachments.size(); j++) {
    ...
    vkAttachment.imageView = vkTexture->getImageView(
        VK_IMAGE_VIEW_TYPE_2D, 1, 1, attachment.level, attachment.layer);
  }
  vkFramebuffer->create(vk(device)->v, vk(renderPass)->v, vkAttachments, w, h,
                        layers);

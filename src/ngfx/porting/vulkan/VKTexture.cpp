...
void VKTexture::create(VKGraphicsContext *ctx, void* data, uint32_t size, VkExtent3D extent, uint32_t arrayLayers, VkFormat format,
       VkImageUsageFlags imageUsageFlags, VkImageViewType imageViewType, bool genMipmaps, VKSamplerCreateInfo *pSamplerCreateInfo,
       uint32_t numSamples) {
    this->ctx = ctx;
    this->w = extent.width; this->h = extent.height; this->d = extent.depth;
    this->arrayLayers = arrayLayers; this->numSamples = numSamples;
    ...
    const std::vector<VkFormat> depthFormats = { VK_FORMAT_D16_UNORM, VK_FORMAT_X8_D24_UNORM_PACK32, VK_FORMAT_D32_SFLOAT };
    depthTexture = std::find(depthFormats.begin(), depthFormats.end(), format) != depthFormats.end();
    this->aspectFlags = depthTexture ? VK_IMAGE_ASPECT_DEPTH_BIT : VK_IMAGE_ASPECT_COLOR_BIT;
    VkImageType imageType;
    if (imageViewType == VK_IMAGE_VIEW_TYPE_1D || imageViewType == VK_IMAGE_VIEW_TYPE_1D_ARRAY) imageType = VK_IMAGE_TYPE_1D;
    else if (imageViewType == VK_IMAGE_VIEW_TYPE_2D || imageViewType == VK_IMAGE_VIEW_TYPE_2D_ARRAY
        || imageViewType == VK_IMAGE_VIEW_TYPE_CUBE) imageType = VK_IMAGE_TYPE_2D;
    else imageType = VK_IMAGE_TYPE_3D;
    this->mipLevels = genMipmaps ? floor(log2(float(glm::min(extent.width, extent.height)))) + 1 : 1;
    ...
    if (imageUsageFlags & IMAGE_USAGE_SAMPLED_BIT) {
        if (genMipmaps) samplerCreateInfo->maxLod = mipLevels;
        if (!sampler) initSampler();
        if (!samplerDescriptorSet) initSamplerDescriptorSet(copyCommandBuffer.v);
    }

    if (imageUsageFlags & IMAGE_USAGE_STORAGE_BIT) {
        if (!storageImageDescriptorSet) initStorageImageDescriptorSet(copyCommandBuffer.v);
    }
    if (imageUsageFlags & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) {
        vkImage.changeLayout(copyCommandBuffer.v, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
                VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT, VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT, aspectFlags);
    }
...
void VKTexture::generateMipmapsFn(VkCommandBuffer cmdBuffer) {
    vkImage.changeLayout(cmdBuffer, VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL,
        ...
        aspectFlags, 0, 1, 0, arrayLayers);

    for (uint32_t j = 1; j<mipLevels; j++) {
        vkImage.changeLayout(cmdBuffer, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
            VK_ACCESS_TRANSFER_WRITE_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT,
            aspectFlags, j, 1, 0, arrayLayers);

        VKBlit::blitImage(cmdBuffer, vkImage.v, j-1, vkImage.v, j,
            {
                { 0, 0, 0 },
                { int32_t(glm::max(w >> (j - 1), 1u)), int32_t(glm::max(h >> (j - 1), 1u)), 1 }
            },
            {
                { 0, 0, 0 },
...
void VKTexture::upload(void* data, uint32_t size, uint32_t x, uint32_t y, uint32_t z, int32_t w, int32_t h, int32_t d, int32_t arrayLayers) {
    auto &copyCommandBuffer = ctx->vkCopyCommandBuffer;
    std::unique_ptr<VKBuffer> stagingBuffer;
    if (data) {
        stagingBuffer.reset(new VKBuffer());
    ...
    copyCommandBuffer.begin();
    uploadFn(copyCommandBuffer.v, data, size, stagingBuffer.get(), x, y, z, w, h, d, arrayLayers);
    if (imageUsageFlags & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) {
        vkImage.changeLayout(copyCommandBuffer.v, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL,
                VK_ACCESS_DEPTH_STENCIL_ATTACHMENT_READ_BIT, VK_PIPELINE_STAGE_EARLY_FRAGMENT_TESTS_BIT, aspectFlags);
    }
...
void VKTexture::uploadFn(VkCommandBuffer cmdBuffer, void* data, uint32_t size, VKBuffer* stagingBuffer,
    ...
    if (data) {
        ...
        VK_TRACE(vkCmdCopyBufferToImage(cmdBuffer, stagingBuffer->v, vkImage.v, vkImage.imageLayout[0],
            uint32_t(bufferCopyRegions.size()), bufferCopyRegions.data()));
    }
    if (data && mipLevels != 1) generateMipmapsFn(cmdBuffer);
}
...
void VKTexture::downloadFn(VkCommandBuffer cmdBuffer, void* data, uint32_t size, VKBuffer* stagingBuffer,
        uint32_t x, uint32_t y, uint32_t z, int32_t w, int32_t h, int32_t d, int32_t arrayLayers) {
    if (w == -1) w = this->w;
    if (h == -1) h = this->h;
    if (d == -1) d = this->d;
    ...
    VK_TRACE(vkCmdCopyImageToBuffer(cmdBuffer, vkImage.v, vkImage.imageLayout[0], stagingBuffer->v,
                                 uint32_t(bufferCopyRegions.size()), bufferCopyRegions.data()));
}

VKTexture::~VKTexture() {
    if (sampler) VK_TRACE(vkDestroySampler(ctx->vkDevice.v, sampler, nullptr));
}

void VKTexture::initSampler() {
    VkResult vkResult;
    V(vkCreateSampler(ctx->vkDevice.v, samplerCreateInfo.get(), nullptr, &sampler));
}
...
void VKTexture::initSamplerDescriptorSet(VkCommandBuffer cmdBuffer) {
    ...
    VkDescriptorSetLayout descriptorSetLayout = ctx->vkDescriptorSetLayoutCache.get(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER);
    VkDescriptorSetAllocateInfo allocInfo = {
            VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO, nullptr, descriptorPool, 1, &descriptorSetLayout
    };
    V(vkAllocateDescriptorSets(ctx->vkDevice.v, &allocInfo, &samplerDescriptorSet));
    VkDescriptorImageInfo descriptorImageInfo = { sampler, vkDefaultImageView->v, vkImage.imageLayout[0] };
    VkWriteDescriptorSet writeDescriptorSet = {
            VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr,
            samplerDescriptorSet, 0, 0, 1, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
            &descriptorImageInfo, nullptr, nullptr
    };
    VK_TRACE(vkUpdateDescriptorSets(ctx->vkDevice.v, 1, &writeDescriptorSet, 0, nullptr));
}
...
void VKTexture::initStorageImageDescriptorSet(VkCommandBuffer cmdBuffer) {
    ...
    vkImage.changeLayout(cmdBuffer, VK_IMAGE_LAYOUT_GENERAL,
        ...
        aspectFlags, 0, mipLevels, 0, this->arrayLayers);
    VkDescriptorPool descriptorPool = ctx->vkDescriptorPool;
    VkDescriptorSetLayout descriptorSetLayout = ctx->vkDescriptorSetLayoutCache.get(VK_DESCRIPTOR_TYPE_STORAGE_IMAGE);
    VkDescriptorSetAllocateInfo allocInfo = {
            VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO, nullptr, descriptorPool, 1, &descriptorSetLayout
    };
    V(vkAllocateDescriptorSets(ctx->vkDevice.v, &allocInfo, &storageImageDescriptorSet));
    VkDescriptorImageInfo descriptorImageInfo = { sampler, vkDefaultImageView->v, vkImage.imageLayout[0] };
    VkWriteDescriptorSet writeDescriptorSet = {
            VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET, nullptr,
            storageImageDescriptorSet, 0, 0, 1, VK_DESCRIPTOR_TYPE_STORAGE_IMAGE,
            &descriptorImageInfo, nullptr, nullptr
    };
    VK_TRACE(vkUpdateDescriptorSets(ctx->vkDevice.v, 1, &writeDescriptorSet, 0, nullptr));
}

void VKTexture::changeLayout(CommandBuffer* commandBuffer, ImageLayout imageLayout) {
...
VKImageView* VKTexture::getImageView(VkImageViewType imageViewType, uint32_t mipLevels, uint32_t arrayLayers,
        uint32_t baseMipLevel, uint32_t baseArrayLayer) {
    VKImageViewCreateInfo imageViewCreateInfo(vkImage.v, imageViewType, vkFormat, aspectFlags, mipLevels, arrayLayers, baseMipLevel, baseArrayLayer);
...
Texture* Texture::create(GraphicsContext* ctx, Graphics* graphics, void* data, PixelFormat format, uint32_t size,
    ...
    if (imageUsageFlags & IMAGE_USAGE_SAMPLED_BIT) {
        ...
        samplerCreateInfo->mipmapMode = (mipFilter == FILTER_LINEAR) ? VK_SAMPLER_MIPMAP_MODE_LINEAR : VK_SAMPLER_MIPMAP_MODE_NEAREST;
    }
    vkTexture->create(vk(ctx), data, size, { w, h, d }, arrayLayers, VkFormat(format),
            imageUsageFlags, VkImageViewType(textureType), genMipmaps, samplerCreateInfo, numSamples);

...
void VKComputePipeline::create(
    ...
    createInfo = {
        ...
        shaderStageCreateInfo, pipelineLayout, 0, 0
    };
    V(vkCreateComputePipelines(device, ctx->vkPipelineCache.v, 1, &createInfo, nullptr, &v));
}
...
ComputePipeline* ComputePipeline::create(GraphicsContext* graphicsContext,
    ...
    uint32_t numDescriptors = cs->descriptors.empty() ? 0 : cs->descriptors.back().set + 1;
    ...
    VKPipelineUtil::parseDescriptors(cs->descriptors, VK_SHADER_STAGE_COMPUTE_BIT, vkDescriptors, descriptorBindings);

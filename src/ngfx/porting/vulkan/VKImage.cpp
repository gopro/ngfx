...
void VKImage::create(VKDevice *vkDevice, VkExtent3D extent, VkFormat format, VkImageUsageFlags usage, VkImageType imageType,
        uint32_t mipLevels, uint32_t arrayLayers, uint32_t numSamples,
        VkImageCreateFlags imageCreateFlags, VkMemoryPropertyFlags memoryPropertyFlags) {
    VKImageCreateInfo p;
    p.format = format; p.extent = extent; p.usage = usage;
...
void VKImage::create(VKDevice *vkDevice, const VKImageCreateInfo& createInfo, VkMemoryPropertyFlags memoryPropertyFlags) {
    this->device = vkDevice->v;
    this->createInfo = createInfo;
    VkResult vkResult;
    V(vkCreateImage(device, &createInfo, nullptr, &v));
    uint32_t mipLevels = createInfo.mipLevels, arrayLayers = createInfo.arrayLayers;
    ...
    memAllloc.memoryTypeIndex = vkDevice->vkPhysicalDevice->getMemoryType(memReqs.memoryTypeBits, memoryPropertyFlags);
    V(vkAllocateMemory(device, &memAllloc, nullptr, &memory));
    V(vkBindImageMemory(device, v, memory, 0));
}

void VKImage::changeLayout(VkCommandBuffer commandBuffer, VkImageLayout newLayout,
       VkAccessFlags dstAccessMask, VkPipelineStageFlags dstStageMask, VkImageAspectFlags aspectMask,
    ...
    VkImageMemoryBarrier imageMemoryBarrier = {
        VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER, nullptr, srcAccessMask, dstAccessMask,
        srcImageLayout, newLayout, VK_QUEUE_FAMILY_IGNORED, VK_QUEUE_FAMILY_IGNORED, v, subresourceRange
    };
    VK_TRACE(vkCmdPipelineBarrier(commandBuffer,
            srcStageMask,
            dstStageMask,
            0,
            0, nullptr,
...
VKImage::~VKImage() {
    if (v) VK_TRACE(vkDestroyImage(device, v, nullptr));

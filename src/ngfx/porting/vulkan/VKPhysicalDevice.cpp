...
void VKPhysicalDevice::getProperties() {
    ...
    if (extCount > 0) {
        ...
        if (vkEnumerateDeviceExtensionProperties(v, nullptr, &extCount, &extensions.front()) == VK_SUCCESS) {
...
void VKPhysicalDevice::selectDevice(VkInstance instance) {
    ...
    auto containsDevice = [&](VkPhysicalDeviceType type, VkPhysicalDevice& deviceResult) -> bool {
        for (int j = 0; j<devicesProperties.size(); j++) {
            auto &deviceProps = devicesProperties[j];
            if (deviceProps.deviceType == type) { deviceResult = physicalDevices[j]; return true; }
        }
        return false;
    };
    VkPhysicalDevice physicalDeviceResult;
    if (containsDevice(PREFERRED_DEVICE_TYPE, physicalDeviceResult)) v = physicalDeviceResult;
    else v = physicalDevices[0];
    vkGetPhysicalDeviceProperties(v, &deviceProperties);
...
uint32_t VKPhysicalDevice::getMemoryType(uint32_t type, VkMemoryPropertyFlags memoryPropertyFlags) {

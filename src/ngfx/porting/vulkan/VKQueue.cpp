...
void VKQueue::create(VKGraphicsContext* ctx, int queueFamilyIndex, int queueIndex) {
    this->ctx = ctx;
...
void VKQueue::present() {
    ...
    const std::vector<VkSemaphore> vkWaitSemaphores = { ctx->vkRenderCompleteSemaphore.v };
    VkPresentInfoKHR presentInfo = {
            VK_STRUCTURE_TYPE_PRESENT_INFO_KHR, nullptr,
            uint32_t(vkWaitSemaphores.size()), vkWaitSemaphores.data(),
            1, &vk(swapChain)->v, &currentImageIndex, nullptr
    };
...
void VKQueue::submit(CommandBuffer* commandBuffer) {
    if (commandBuffer == &ctx->vkComputeCommandBuffer) {
        ...
        submit(commandBuffer, VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT, {}, {}, nullptr);
    }
    else {
...
void VKQueue::submit(CommandBuffer * commandBuffer, VkPipelineStageFlags waitStageMask, 
    ...
    VkSubmitInfo submitInfo = {
        ...
        uint32_t(vkWaitSemaphores.size()), vkWaitSemaphores.data(),
        &waitStageMask,
        1, &vk(commandBuffer)->v,
        uint32_t(vkSignalSemaphores.size()), vkSignalSemaphores.data()
    };
    V(vkQueueSubmit(v, 1, &submitInfo, waitFence ? vk(waitFence)->v : VK_NULL_HANDLE));
}

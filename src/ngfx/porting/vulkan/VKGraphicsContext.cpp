...
void VKGraphicsContext::create(const char* appName, bool enableDepthStencil, bool debug) {
    this->debug = debug;
    vkInstance.create(appName, "Graphics Abstraction Engine", 0, debug);
    auto instance = vkInstance.v;
    if (debug) VKDebugMessenger::inst.create(instance);
...
VKGraphicsContext::~VKGraphicsContext() {
    ...
    if (debug) VKDebugMessenger::inst.destroy();
}
...
void VKGraphicsContext::initDescriptorPool() {
    ...
    descriptorPoolCreateInfo = {
            ...
            uint32_t(descriptorPoolSizes.size()), descriptorPoolSizes.data()
    };
    V(vkCreateDescriptorPool(vkDevice.v, &descriptorPoolCreateInfo, nullptr, &vkDescriptorPool));
};
...
void VKGraphicsContext::createSwapchainFramebuffers(int w, int h) {
    ...
    for (uint32_t i = 0; i < vkSwapchainFramebuffers.size(); i++) {
        ...
        if (numSamples != 1) attachments.push_back({
            &vkMultisampleColorImageView,
            { vkMultisampleColorImage.createInfo.usage }
        });
        attachments.push_back({
            &vkSwapchain->imageViews[i],
        ...
        if (enableDepthStencil) {
            ...
            attachments.push_back({
                &vkDepthStencilImageView,
                { vkDepthStencilImage.createInfo.usage }
            });
        }
        vkSwapchainFramebuffers[i].create(vkDevice.v, vkDefaultRenderPass->v, attachments, w, h);
    }
}
...
void VKGraphicsContext::initFences(VkDevice device) {
    ...
    for (auto& fence: vkWaitFences) fence.create(device, VK_FENCE_CREATE_SIGNALED_BIT);
...
void VKGraphicsContext::setSurface(Surface* surface) {
    ...
    if (surface && numSamples != 1) {
        ...
        msColorImageCreateInfo.usage = VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT;
        vkMultisampleColorImage.create(&vkDevice, msColorImageCreateInfo); //TODO: use VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT on mobile GPU
        vkMultisampleColorImageView.create(vkDevice.v, vkMultisampleColorImage.v, VK_IMAGE_VIEW_TYPE_2D, VkFormat(surfaceFormat));
    }
    if (surface && enableDepthStencil) {
        vkDepthStencilImage.create(&vkDevice, { surface->w, surface->h, 1}, vkPhysicalDevice.depthFormat,
            VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT);
        vkDepthStencilImageView.create(vkDevice.v, vkDepthStencilImage.v, VK_IMAGE_VIEW_TYPE_2D,
        ...
        if (numSamples != 1) {
            ...
            msDepthImageCreateInfo.usage = VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT | VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT;
            vkMultisampleDepthImage.create(&vkDevice, msDepthImageCreateInfo); //TODO: use VK_MEMORY_PROPERTY_LAZILY_ALLOCATED_BIT on mobile GPU
            vkMultisampleDepthImageView.create(vkDevice.v, vkMultisampleDepthImage.v,
...
void VKGraphicsContext::createBindings() {
    device = &vkDevice;
    queue = &vkQueue;
    defaultRenderPass = offscreen ? vkDefaultOffscreenRenderPass : vkDefaultRenderPass;

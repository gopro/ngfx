...
namespace ngfx {
    ...
    public:
        void create(VKGraphicsContext *ctx, void* data, uint32_t size, VkExtent3D extent, uint32_t arrayLayers, VkFormat format,
        ...
        VKImageView* getImageView(VkImageViewType imageViewType, uint32_t mipLevels, uint32_t arrayLayers,
    ...
    private:
        ...
        void initSampler();
        void uploadFn(VkCommandBuffer cmdBuffer, void* data, uint32_t size, VKBuffer* stagingBuffer,
                uint32_t x = 0, uint32_t y = 0, uint32_t z = 0,
                int32_t w = -1, int32_t h = -1, int32_t d = -1, int32_t arrayLayers = -1);
        void downloadFn(VkCommandBuffer cmdBuffer, void* data, uint32_t size, VKBuffer* stagingBuffer,
                uint32_t x = 0, uint32_t y = 0, uint32_t z = 0,
                int32_t w = -1, int32_t h = -1, int32_t d = -1, int32_t arrayLayers = -1);

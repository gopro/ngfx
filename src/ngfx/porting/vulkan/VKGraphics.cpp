...
void VKGraphics::beginRenderPass(CommandBuffer* commandBuffer, RenderPass* renderPass, Framebuffer* framebuffer,
    ...
    for (uint32_t j = 0; j<vkAttachmentInfos.size(); j++) {
        ...
        bool depthStencilAttachment = info.imageUsageFlags & VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
        if (depthStencilAttachment) clearValues[j].depthStencil = { clearDepth, clearStencil };
        else clearValues[j].color = { { clearColor[0], clearColor[1], clearColor[2], clearColor[3] } };
    }
    VkRenderPassBeginInfo renderPassBeginInfo = {
        VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO, nullptr,
        vk(renderPass)->v, vk(framebuffer)->v, { { 0, 0 }, { vk(framebuffer)->w, vk(framebuffer)->h } },
...
void VKGraphics::bindComputePipeline(CommandBuffer* commandBuffer, ComputePipeline* computePipeline) {
    VK_TRACE(vkCmdBindPipeline(vk(commandBuffer)->v, VK_PIPELINE_BIND_POINT_COMPUTE, vk(computePipeline)->v));
    currentPipeline = computePipeline;
}

void VKGraphics::bindGraphicsPipeline(CommandBuffer* commandBuffer, GraphicsPipeline* graphicsPipeline) {
    VK_TRACE(vkCmdBindPipeline(vk(commandBuffer)->v, VK_PIPELINE_BIND_POINT_GRAPHICS, vk(graphicsPipeline)->v));
    currentPipeline = graphicsPipeline;
}

void VKGraphics::bindTexture(CommandBuffer* commandBuffer, Texture* texture, uint32_t set) {
    auto vkTexture = vk(texture);
    VkPipelineLayout pipelineLayout;
    VkPipelineBindPoint pipelineBindPoint;
    VkDescriptorSet *descriptorSet = nullptr;
    if (VKGraphicsPipeline* graphicsPipeline = dynamic_cast<VKGraphicsPipeline*>(currentPipeline)) {
    ...
    else if (VKComputePipeline* computePipeline = dynamic_cast<VKComputePipeline*>(currentPipeline)) {
    ...
    VK_TRACE(vkCmdBindDescriptorSets(vk(commandBuffer)->v, pipelineBindPoint, pipelineLayout,
...
void VKGraphics::bindUniformBuffer(CommandBuffer* commandBuffer, Buffer* buffer, uint32_t set, ShaderStageFlags shaderStageFlags) {
    bindBufferFN0(commandBuffer, buffer, set, currentPipeline, &vk(buffer)->getUboDescriptorSet(shaderStageFlags));
}

void VKGraphics::bindStorageBuffer(CommandBuffer* commandBuffer, Buffer* buffer, uint32_t set, ShaderStageFlags shaderStageFlags) {
...
void VKGraphics::draw(CommandBuffer* commandBuffer, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance) {
    VK_TRACE(vkCmdDraw(vk(commandBuffer)->v, vertexCount, instanceCount, firstVertex, firstInstance));
}
void VKGraphics::drawIndexed(CommandBuffer* cmdBuffer, uint32_t indexCount, uint32_t instanceCount, uint32_t firstIndex, int32_t vertexOffset, uint32_t firstInstance) {
    VK_TRACE(vkCmdDrawIndexed(vk(cmdBuffer)->v, indexCount, instanceCount, firstIndex, vertexOffset, firstInstance));
}

void VKGraphics::setViewport(CommandBuffer* commandBuffer, Rect2D r) {
    viewport = r;
    VkViewport vkViewport = { float(r.x), float(r.y), float(r.w), float(r.h), 0.0f, 1.0f };
...
void VKGraphics::setScissor(CommandBuffer* commandBuffer, Rect2D r) {
    ...
    auto& v = viewport;
    VkRect2D vkScissorRect = {{ r.x, int32_t(v.h) - r.y - int32_t(r.h) }, { r.w, r.h }};
#else

...
void VKShaderModule::initFromFile(VkDevice device, const std::string &filename) {
    File file;
...
void VKShaderModule::initFromByteCode(VkDevice device, void* data, uint32_t size) {
    this->device = device;
    VkResult vkResult;
    VkShaderModuleCreateInfo moduleCreateInfo = {
            VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, nullptr, 0, size, (const uint32_t*)data
    };
    V(vkCreateShaderModule(this->device, &moduleCreateInfo, NULL, &v));
}

VKShaderModule::~VKShaderModule() {
    if (v) VK_TRACE(vkDestroyShaderModule(device, v, nullptr));
}

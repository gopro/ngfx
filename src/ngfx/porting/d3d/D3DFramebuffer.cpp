...
Framebuffer* Framebuffer::create(Device* device, RenderPass* renderPass,
    ...
    for (uint32_t j = 0; j < attachments.size(); j++) {
        ...
        bool depthStencilAttachment = d3dTexture->imageUsageFlags & IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT;
        d3dAttachment = { d3dTexture->v.Get(),
            depthStencilAttachment ? 
            d3dTexture->dsvDescriptor.cpuHandle : 
            d3dTexture->getRtvDescriptor(attachment.level, attachment.layer, layers).cpuHandle,

...
D3DBlitOp::D3DBlitOp(D3DGraphicsContext* ctx,
        D3DTexture* srcTexture, uint32_t srcLevel, D3DTexture* dstTexture, uint32_t dstLevel,
        Region srcRegion, Region dstRegion,
        uint32_t srcBaseLayer, uint32_t srcLayerCount,
        uint32_t dstBaseLayer, uint32_t dstLayerCount): ctx(ctx), srcTexture(srcTexture), srcLevel(srcLevel),
        dstTexture(dstTexture), dstLevel(dstLevel), srcRegion(srcRegion), dstRegion(dstRegion), 
        srcBaseLayer(srcBaseLayer), srcLayerCount(srcLayerCount),
        dstBaseLayer(dstBaseLayer), dstLayerCount(dstLayerCount) {
    outputFramebuffer.reset(
        (D3DFramebuffer*)Framebuffer::create(
...
void D3DBlitOp::apply(D3DGraphicsContext* ctx, D3DCommandList* cmdList, D3DGraphics* graphics) {

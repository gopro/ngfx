...
void D3DSwapchain::create(D3DGraphicsContext* ctx, D3DSurface* surface) {
    ...
    DXGI_SWAP_CHAIN_DESC1 swapChainDesc = {
        w, h, DXGI_FORMAT_R8G8B8A8_UNORM, FALSE,
        { 1, 0 }, DXGI_USAGE_RENDER_TARGET_OUTPUT, numImages,
        DXGI_SCALING_STRETCH, DXGI_SWAP_EFFECT_FLIP_DISCARD, 
        DXGI_ALPHA_MODE_UNSPECIFIED, 0
    };
    ComPtr<IDXGISwapChain1> swapchain;
    V(d3dFactory->CreateSwapChainForHwnd(
        d3dCommandQueue, surface->v, &swapChainDesc,
...
void D3DSwapchain::createSwapchainRenderTargetViews(uint32_t w, uint32_t h) {
    ...
    for (UINT n = 0; n < numImages; n++) {
        ...
        D3D_TRACE(d3dDevice->CreateRenderTargetView(renderTargets[n].Get(), nullptr, rtvDescriptorHeap->handle.cpuHandle));

...
void D3DBuffer::create(D3DGraphicsContext* ctx, const void* data, uint32_t size,
    ...
    if (bufferUsageFlags & BUFFER_USAGE_TRANSFER_SRC_BIT) heapType = D3D12_HEAP_TYPE_UPLOAD;
    else heapType = D3D12_HEAP_TYPE_DEFAULT;
    D3D12_RESOURCE_FLAGS resourceFlags = D3D12_RESOURCE_FLAG_NONE;
    if (bufferUsageFlags & BUFFER_USAGE_STORAGE_BUFFER_BIT) resourceFlags |= D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
    create(ctx, data, size, heapType, resourceFlags);
}

void D3DBuffer::create(D3DGraphicsContext* ctx, const void* data, uint32_t size,
        D3D12_HEAP_TYPE heapType, D3D12_RESOURCE_FLAGS resourceFlags, D3D12_RESOURCE_STATES initialResourceState) {
    HRESULT hResult;
    this->ctx = ctx;
...
void* D3DBuffer::map() {
    if (heapType == D3D12_HEAP_TYPE_DEFAULT) {
        ...
        D3D_TRACE(copyCommandList.v.Get()->CopyBufferRegion(d3dReadbackBuffer->v.Get(), 0, v.Get(), 0, size));
        CD3DX12_RESOURCE_BARRIER resourceBarrier = CD3DX12_RESOURCE_BARRIER::Transition(
            v.Get(), D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_GENERIC_READ
        );
        D3D_TRACE(copyCommandList.v.Get()->ResourceBarrier(1, &resourceBarrier));
        copyCommandList.end();
...
void D3DBuffer::upload(const void* data, uint32_t size, uint32_t offset) {
    if (heapType == D3D12_HEAP_TYPE_DEFAULT) {
        ...
        if (data) {
            ...
            D3D_TRACE(copyCommandList.v.Get()->CopyBufferRegion(v.Get(), 0, stagingBuffer.v.Get(), 0, size));
        }
        CD3DX12_RESOURCE_BARRIER resourceBarrier = CD3DX12_RESOURCE_BARRIER::Transition(

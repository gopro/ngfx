...
void D3DGraphics::bindGraphicsPipeline(CommandBuffer* commandBuffer, GraphicsPipeline* graphicsPipeline) {
    auto d3dCommandList = d3d(commandBuffer)->v;
    auto d3dGraphicsPipeline = d3d(graphicsPipeline);
    D3D_TRACE(d3dCommandList->SetPipelineState(d3dGraphicsPipeline->d3dPipelineState.Get()));
    D3D_TRACE(d3dCommandList->IASetPrimitiveTopology(d3dGraphicsPipeline->d3dPrimitiveTopology));
    D3D_TRACE(d3dCommandList->SetGraphicsRootSignature(d3dGraphicsPipeline->d3dRootSignature.Get()));
    currentPipeline = graphicsPipeline;
}

void D3DGraphics::bindUniformBuffer(CommandBuffer* commandBuffer, Buffer* buffer, uint32_t binding, ShaderStageFlags shaderStageFlags) {
    ...
    if (D3DGraphicsPipeline* graphicsPipeline = dynamic_cast<D3DGraphicsPipeline*>(currentPipeline)) {
        D3D_TRACE(d3dCommandList->SetGraphicsRootConstantBufferView(binding, d3dBuffer->v->GetGPUVirtualAddress()));
    }
    else if (D3DComputePipeline* computePipeline = dynamic_cast<D3DComputePipeline*>(currentPipeline)) {
        D3D_TRACE(d3dCommandList->SetComputeRootConstantBufferView(binding, d3dBuffer->v->GetGPUVirtualAddress()));
    }
}
...
void D3DGraphics::draw(CommandBuffer* cmdBuffer, uint32_t vertexCount, uint32_t instanceCount, uint32_t firstVertex, uint32_t firstInstance) {
    D3D_TRACE(d3d(cmdBuffer)->v->DrawInstanced(vertexCount, instanceCount, firstVertex, firstInstance));
}
void D3DGraphics::drawIndexed(CommandBuffer* cmdBuffer, uint32_t indexCount, uint32_t instanceCount, 
        uint32_t firstIndex, int32_t vertexOffset, uint32_t firstInstance) {
    D3D_TRACE(d3d(cmdBuffer)->v->DrawIndexedInstanced(indexCount, instanceCount, firstIndex, vertexOffset, firstInstance));
}
void D3DGraphics::setViewport(CommandBuffer* cmdBuffer, Rect2D r) {
    viewport = r;
    D3D12_VIEWPORT d3dViewport = { float(r.x), float(r.y), float(r.w), float(r.h), 0.0f, 1.0f };
...
void D3DGraphics::setScissor(CommandBuffer* cmdBuffer, Rect2D r) {
    ...
    auto& v = viewport;
    D3D12_RECT d3dScissorRect = { long(r.x), long(v.h - r.y - r.h), long(r.x + r.w), long(v.h - r.y) };
#else
    D3D12_RECT d3dScissorRect = { long(r.x), long(r.y), long(r.x + r.w), long(r.y + r.h) };
#endif

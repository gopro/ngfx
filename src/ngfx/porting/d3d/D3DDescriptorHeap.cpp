...
void D3DDescriptorHeap::create(ID3D12Device* d3dDevice, D3D12_DESCRIPTOR_HEAP_TYPE type, UINT maxDescriptors,
        D3D12_DESCRIPTOR_HEAP_FLAGS flags) {
    HRESULT hResult;
    this->type = type; this->maxDescriptors = maxDescriptors;
    D3D12_DESCRIPTOR_HEAP_DESC desc = {
       type, maxDescriptors, flags, 0
    };
    V(d3dDevice->CreateDescriptorHeap(&desc, IID_PPV_ARGS(&v)));
    D3D_TRACE(handle.cpuHandle = v->GetCPUDescriptorHandleForHeapStart());
    D3D_TRACE(handle.descriptorSize = d3dDevice->GetDescriptorHandleIncrementSize(type));

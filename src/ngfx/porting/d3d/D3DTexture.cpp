...
void D3DTexture::create(D3DGraphicsContext* ctx, D3DGraphics* graphics, void* data, uint32_t size, uint32_t w, uint32_t h, uint32_t d, uint32_t arrayLayers,
    DXGI_FORMAT format, ImageUsageFlags usageFlags, TextureType textureType, bool genMipmaps, uint32_t numSamples, const D3DSamplerDesc& samplerDesc) {
    this->ctx = ctx;
    ...
    if (imageUsageFlags & IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) resourceFlags |= D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL;
    ...
    if (imageUsageFlags & IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) {
        ...
        dsvDesc.ViewDimension = (numSamples > 1) ? D3D12_DSV_DIMENSION_TEXTURE2DMS : D3D12_DSV_DIMENSION_TEXTURE2D;
        D3D_TRACE(d3dDevice->CreateDepthStencilView(v.Get(), &dsvDesc, dsvDescriptorHeap->handle.cpuHandle));
...
D3DDescriptorHandle D3DTexture::getSamplerDescriptor(D3D12_FILTER filter) {
    for (auto& samplerData : samplerDescriptorCache) {
        if (samplerData.desc.Filter == filter) return samplerData.handle;
    }
    //Create sampler
    ...
    auto d3dDevice = ctx->d3dDevice.v.Get();
    D3D_TRACE(d3dDevice->CreateSampler(&samplerDesc, samplerDescriptorHeap.handle.cpuHandle));
...
D3DDescriptorHandle D3DTexture::getSrvDescriptor(uint32_t baseMipLevel, uint32_t numMipLevels) {
    for (auto& srvData : srvDescriptorCache) {
        if (textureType == TEXTURE_TYPE_2D && srvData.desc.Texture2D.MostDetailedMip == baseMipLevel 
            && srvData.desc.Texture2D.MipLevels == numMipLevels) return srvData.handle;
        else if (textureType == TEXTURE_TYPE_2D_ARRAY && srvData.desc.Texture2DArray.MostDetailedMip == baseMipLevel
            && srvData.desc.Texture2DArray.MipLevels == numMipLevels) return srvData.handle;
        if (textureType == TEXTURE_TYPE_3D && srvData.desc.Texture3D.MostDetailedMip == baseMipLevel
            && srvData.desc.Texture3D.MipLevels == numMipLevels) return srvData.handle;
        else if (textureType == TEXTURE_TYPE_CUBE && srvData.desc.TextureCube.MostDetailedMip == baseMipLevel
            && srvData.desc.TextureCube.MipLevels == numMipLevels) return srvData.handle;
    ...
    auto d3dDevice = ctx->d3dDevice.v.Get();
    D3D_TRACE(d3dDevice->CreateShaderResourceView(v.Get(), &srvDesc,
        cbvSrvUavDescriptorHeap.handle.cpuHandle));
    SrvData srvData;
...
D3D12_RENDER_TARGET_VIEW_DESC getRtvDesc(TextureType textureType, DXGI_FORMAT format, uint32_t numSamples, 
    ...
    else if (textureType == TEXTURE_TYPE_2D_ARRAY || textureType == TEXTURE_TYPE_CUBE) {
        if (numSamples > 1) {
...
D3DDescriptorHandle D3DTexture::getRtvDescriptor(uint32_t level, uint32_t baseLayer, uint32_t layerCount) {
    for (auto& rtvData : rtvDescriptorCache) {
        if (textureType == TEXTURE_TYPE_2D && numSamples > 1) return rtvData.handle;
        else if (textureType == TEXTURE_TYPE_2D && rtvData.desc.Texture2D.MipSlice == level) return rtvData.handle;
        else if ((textureType == TEXTURE_TYPE_2D_ARRAY || textureType == TEXTURE_TYPE_CUBE) && numSamples > 1 &&
    ...
    auto d3dDevice = ctx->d3dDevice.v.Get();
    D3D12_RENDER_TARGET_VIEW_DESC rtvDesc = getRtvDesc(textureType, DXGI_FORMAT(format), numSamples, level, baseLayer, layerCount);
    D3D_TRACE(d3dDevice->CreateRenderTargetView(v.Get(), &rtvDesc, rtvDescriptorHeap->handle.cpuHandle));
    RtvData rtvData;
...
void D3DTexture::generateMipmapsFn(D3DCommandList* cmdList) {
    ...
    for (uint32_t j = 1; j < mipLevels; j++) {
        D3DBlitOp op(ctx, this, j - 1, this, j,
            {
                { 0, 0, 0 },
...
void D3DTexture::upload(void* data, uint32_t size, uint32_t x, uint32_t y, uint32_t z, int32_t w, int32_t h, int32_t d, int32_t arrayLayers) {
    ...
    if (data) {
        ...
        D3D_TRACE(stagingBufferSize = GetRequiredIntermediateSize(v.Get(), 0, arrayLayers));
        stagingBuffer.reset(new D3DBuffer());
        stagingBuffer->create(ctx, nullptr, uint32_t(stagingBufferSize), D3D12_HEAP_TYPE_UPLOAD);
    }
    copyCommandList.begin();
    uploadFn(&copyCommandList, data, size, stagingBuffer.get(), x, y, z, w, h, d, arrayLayers);
    ...
    if (data && mipLevels != 1) {
        ...
        HRESULT hResult;
        auto d3dDevice = ctx->d3dDevice.v.Get();
        V(d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&cmdAllocator)));
...
void D3DTexture::uploadFn(D3DCommandList* cmdList, void* data, uint32_t size, D3DBuffer* stagingBuffer,
        uint32_t x, uint32_t y, uint32_t z, int32_t w, int32_t h, int32_t d, int32_t arrayLayers) {
    if (data) {
        if (x != 0 || y != 0 || z != 0) NGFX_LOG_TRACE("TODO: support sub-region update");
        ...
        uint64_t bufferSize = UpdateSubresources(cmdList->v.Get(), v.Get(), stagingBuffer->v.Get(), 0, 0, arrayLayers, textureData.data());
        assert(bufferSize);
    }
}
...
void D3DTexture::downloadFn(D3DCommandList* cmdList, D3DReadbackBuffer& readbackBuffer, D3D12_BOX &srcRegion,
    ...
    D3D_TRACE(cmdList->v->CopyTextureRegion(&dstLocation, 0, 0, 0, &srcLocation, &srcRegion));
    D3D12_RESOURCE_STATES resourceState = (imageUsageFlags & IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT) ?
        D3D12_RESOURCE_STATE_DEPTH_WRITE :
        D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE | D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE;
...
Texture* Texture::create(GraphicsContext* ctx, Graphics* graphics, void* data, PixelFormat format, uint32_t size, uint32_t w, uint32_t h, uint32_t d, uint32_t arrayLayers,
    ImageUsageFlags imageUsageFlags, TextureType textureType, bool genMipmaps, FilterMode minFilter, FilterMode magFilter, FilterMode mipFilter, uint32_t numSamples) {
    ...
    d3dTexture->create(
        d3d(ctx), (D3DGraphics*)graphics, data, size, w, h, d, arrayLayers, 
        DXGI_FORMAT(format), imageUsageFlags, textureType, genMipmaps, numSamples, samplerDesc
    );
    return d3dTexture;
}

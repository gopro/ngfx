...
namespace ngfx {
	...
	public:
		void create(D3DGraphicsContext* ctx, D3DGraphics* graphics, void* data, uint32_t size, uint32_t w, uint32_t h, uint32_t d, uint32_t arrayLayers,
		...
		struct RtvData { D3D12_RENDER_TARGET_VIEW_DESC desc; D3DDescriptorHandle handle; };
		std::vector<RtvData> rtvDescriptorCache;
		struct SamplerData { D3DSamplerDesc desc; D3DDescriptorHandle handle; };
		...
		struct SrvData { D3D12_SHADER_RESOURCE_VIEW_DESC desc; D3DDescriptorHandle handle; };
		std::vector<SrvData> srvDescriptorCache;
		D3D12_RESOURCE_DESC resourceDesc;
	private:
		void downloadFn(D3DCommandList* cmdList, D3DReadbackBuffer& readbackBuffer, D3D12_BOX& srcRegion,
			D3D12_PLACED_SUBRESOURCE_FOOTPRINT& dstFootprint);
		void uploadFn(D3DCommandList* cmdList, void* data, uint32_t size, D3DBuffer* stagingBuffer,
			uint32_t x = 0, uint32_t y = 0, uint32_t z = 0, int32_t w = -1, int32_t h = -1, int32_t d = -1, int32_t arrayLayers = -1);

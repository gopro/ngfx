...
D3D12_PRIMITIVE_TOPOLOGY_TYPE D3DGraphicsPipeline::getPrimitiveTopologyType(
...
void D3DGraphicsPipeline::create(D3DGraphicsContext* ctx, const State& state,
    ...
    createRootSignature(rootParameters, D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT);
    ...
    D3D12_RENDER_TARGET_BLEND_DESC renderTargetBlendDesc = {
        ...
        state.blendSrcColorFactor, state.blendDstColorFactor, state.blendColorOp,
        state.blendSrcAlphaFactor, state.blendDstAlphaFactor, state.blendAlphaOp,
        D3D12_LOGIC_OP_NOOP,
    ...
    desc.PrimitiveTopologyType = getPrimitiveTopologyType(state.primitiveTopology);
    desc.NumRenderTargets = state.numColorAttachments;
    for (uint32_t j = 0; j<desc.NumRenderTargets; j++) desc.RTVFormats[j] = colorFormat;
    desc.SampleDesc.Count = state.numSamples;

    V(d3dDevice->CreateGraphicsPipelineState(&desc, IID_PPV_ARGS(&d3dPipelineState)));
}
...
GraphicsPipeline* GraphicsPipeline::create(GraphicsContext* graphicsContext, const State& state,
    ...
    auto getAlphaBlendFactor = [](BlendFactor blendFactor) -> BlendFactor {
        switch (blendFactor) {
        case BLEND_FACTOR_SRC_COLOR: return BLEND_FACTOR_SRC_ALPHA; break;
        case BLEND_FACTOR_DST_COLOR: return BLEND_FACTOR_DST_ALPHA; break;
        case BLEND_FACTOR_ONE_MINUS_SRC_COLOR: return BLEND_FACTOR_ONE_MINUS_SRC_ALPHA; break;
        case BLEND_FACTOR_ONE_MINUS_DST_COLOR: return BLEND_FACTOR_ONE_MINUS_DST_ALPHA; break;
    ...
    struct SemanticData { string name; uint32_t index; };
    ...
    for (int j = 0; j < vs->attributes.size(); j++) {
        ...
        d3dVertexInputAttributes[j] = { 
            semanticName.c_str(), semanticIndex, DXGI_FORMAT(va.format), 
            binding, offset, inputRate, (inputRate == D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA ) ? UINT(0) : UINT(1)
        };
    ...
    d3dGraphicsPipeline->create(d3d(graphicsContext), d3dState, d3dRootParams, 
		d3dVertexInputAttributes, shaders, DXGI_FORMAT(colorFormat), DXGI_FORMAT(depthFormat));

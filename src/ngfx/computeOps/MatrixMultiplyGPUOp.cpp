...
void MatrixMultiplyGPUOp::apply(CommandBuffer* commandBuffer, Graphics* graphics) {
...
void MatrixMultiplyGPUOp::update(MatrixParam src0, MatrixParam src1) {
    ...
    UboData uboData = { int32_t(src0.w), int32_t(src0.h), int32_t(src1t.w), int32_t(src1t.h), int32_t(dst.w), int32_t(dst.h) };
    bUbo.reset(createUniformBuffer(ctx, &uboData, sizeof(uboData)));
    bSrc0.reset(createStorageBuffer(ctx, src0.data, src0.w * src0.h * sizeof(float)));
    bSrc1.reset(createStorageBuffer(ctx, src1t.data, src1.w * src1.h * sizeof(float)));
...
void MatrixMultiplyGPUOp::createPipeline() {
    ...
    computePipeline = ComputePipeline::create(ctx,

...
#define VF_ITEM(s,count, elementSize) { #s, { s, count, elementSize } }
...
static map<string, VertexInputRate> vertexInputRateMap = {
...
static void parseDescriptors(ifstream& in, vector<ShaderModule::DescriptorInfo> &descs) {
    string token;
    int numDescriptors;
...
static void parseBufferMemberInfos(ifstream &in, ShaderModule::BufferMemberInfos &memberInfos) {
...
static void parseBufferInfos(ifstream &in, string key, ShaderModule::BufferInfos &bufferInfos,
...
void ShaderModule::initBindings(std::ifstream& in, ShaderStageFlags shaderStages) {
    parseDescriptors(in, descriptors);
    parseBufferInfos(in, "UNIFORM_BUFFER_INFOS", uniformBufferInfos, shaderStages);
    parseBufferInfos(in, "SHADER_STORAGE_BUFFER_INFOS", shaderStorageBufferInfos, shaderStages);
}

void ShaderModule::initBindings(const std::string& filename, ShaderStageFlags shaderStages) {
    ifstream in(filename);
    if (!in.is_open()) NGFX_ERR("cannot open file: %s", filename.c_str());

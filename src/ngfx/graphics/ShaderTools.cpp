...
int ShaderTools::cmd(string str) {
    if (verbose) { NGFX_LOG(">> %s", str.c_str()); }
...
int ShaderTools::preprocess(const string &src, const string &dataPath, string &dst) {
    dst = "";
...
int ShaderTools::compileShaderGLSL(const string &src, shaderc_shader_kind shaderKind, const MacroDefinitions &defines, string &spv, bool verbose,
...
int ShaderTools::removeUnusedVariablesGLSL(const std::string &src, shaderc_shader_kind shaderKind, const MacroDefinitions &defines, std::string &dst) {
    int ret = 0;
    string spv;
...
int ShaderTools::patchShaderLayoutsGLSL(const string &src, string &dst) {
    ...
    while (std::getline(sstream, line)) {
        ...
        if (matchLayout) {
            dst += g[1].str() + "layout(" + g[2].str() + "set = " + g[3].str() + ", binding = 0" + g[4].str() + ")" + g[5].str() + "\n";
        }
        else {
            dst += line + "\n";
        }
...
int ShaderTools::compileShaderMSL(const string &file, const MacroDefinitions &defines, string outDir, vector<string> &outFiles) {
...
int ShaderTools::convertShader(const string &file, const string &extraArgs, string outDir, Format fmt, vector<string> &outFiles) {
    ...
    if (result == 0)
        NGFX_LOG("converted file: %s to %s", inFileName.c_str(), outFileName.c_str());
    else
...
int ShaderTools::patchShaderReflectionDataMSL(const std::string &glslReflect, const std::string &ext, const std::string &msl, std::string &mslReflect) {
    auto glslReflectJson = json::parse(glslReflect);
    MetalReflectData metalReflectData;
    if (ext == ".vert") {
       metalReflectData.attributes = RegexUtil::findAll(regex("([^\\s]*)[\\s]*([^\\s]*)[\\s]*\\[\\[attribute\\(([0-9]+)\\)\\]\\]"), msl);
    }
    metalReflectData.buffers = RegexUtil::findAll(regex("([^\\s]*)[\\s]*([^\\s]*)[\\s]*\\[\\[buffer\\(([0-9]+)\\)\\]\\]"), msl);
    metalReflectData.textures = RegexUtil::findAll(regex("([^\\s]*)[\\s]*([^\\s]*)[\\s]*\\[\\[texture\\(([0-9]+)\\)\\]\\]"), msl);
...
int ShaderTools::patchShaderReflectionDataHLSL(const std::string &glslReflect, const std::string &ext, const std::string &hlsl, std::string &hlslReflect) {
    auto glslReflectJson = json::parse(glslReflect);
...
int ShaderTools::genShaderReflectionGLSL(const string &, const string &ext, const string &spv, string &glslMap) {
...
int ShaderTools::genShaderReflectionMSL(const string &msl, const string &ext, const string &spv, string &mslMap) {
    string glslReflect;
    genShaderReflectionGLSL("", ext, spv, glslReflect);
    return patchShaderReflectionDataMSL(glslReflect, ext, msl, mslMap);
}

int ShaderTools::genShaderReflectionHLSL(const string& hlsl, const string& ext, const string& spv, string& hlslMap) {
    string glslReflect;
    genShaderReflectionGLSL("", ext, spv, glslReflect);
...
string ShaderTools::parseReflectionData(const json &reflectData, string ext) {
    ...
    std::function<void(const json&, json&, uint32_t, string)> parseMembers = [&](const json &membersData, json &members, uint32_t baseOffset = 0, string baseName = "") {
        for (const json &memberData: membersData) {
            ...
            else NGFX_ERR("unrecognized type: {memberType}");
        }
    };
...
int ShaderTools::generateShaderMapGLSL(const string &file, string outDir, vector<string> &outFiles) {
    ...
    string glslFileName = fs::path(outDir + "/" + filename).make_preferred().string();
    string spvFileName = fs::path(outDir + "/" + filename + ".spv").make_preferred().string();
    string glslMapFileName = fs::path(outDir + "/" + filename + ".map").make_preferred().string();
...
int ShaderTools::generateShaderMapMSL(const string &file, string outDir, vector<string> &outFiles) {
    ...
    string mslFileName = fs::path(outDir + "/" + glslFilename + ".metal").make_preferred().string();
    string spvFileName = fs::path(outDir + "/" + glslFilename + ".spv").make_preferred().string();
    string mslMapFileName = fs::path(outDir + "/" + glslFilename + ".metal.map").make_preferred().string();
...
int ShaderTools::generateShaderMapHLSL(const string &file, string outDir, vector<string> &outFiles) {
    ...
    string hlslFileName = fs::path(outDir + "/" + glslFilename + ".hlsl").make_preferred().string();
    string spvFileName = fs::path(outDir + "/" + glslFilename + ".spv").make_preferred().string();
    string hlslMapFileName = fs::path(outDir + "/" + glslFilename + ".hlsl.map").make_preferred().string();
    if (!FileUtil::srcFileNewerThanOutFile(hlslFileName, hlslMapFileName)) {
        outFiles.push_back(hlslMapFileName);
        return 0;
    }

    string hlsl = readFile(hlslFileName), spv = readFile(spvFileName), hlslReflect;
...
vector<string> ShaderTools::compileShaders(const vector<string> &files, string outDir, Format fmt, const MacroDefinitions &defines, int flags) {
    ...
    for (const string &file: files) {
        ...
        else if (fmt == FORMAT_MSL) compileShaderMSL(file, defines, outDir, outFiles);
